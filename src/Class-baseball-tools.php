...
Namespace Baseball_tools;
  	...
  	*  This library requires includes the open source database class ezSQL by Justin Vincent 
  ...
  class Baseball_tools {
    ...
    protected $death_year;
    protected $death_month;
    protected $death_day;
    protected $death_country;
    protected $death_state;
    protected $death_city;
    ...
    protected $throws;
    protected $debut;
    protected $final_game;
    protected $college;
    protected $lahman_4_0_id;
    protected $lahman_4_5_id;
    protected $retro_id;
    protected $holtz_id;
    protected $bbref_id;
    ...
    function __construct( $db_info ) {
      ...
      $this->get_db( $this->db_info );
      $this->current_year = max( $this->db->get_var( "select max( yearID ) as year from Batting" ) , $this->db->get_var( "select max( yearID ) as year from Pitching" ) );
    }


    protected function get_db( $db_info ){
      if ( ! $db_info->server ) {
        $db_info->server = 'localhost';
      }
      $db = new \ezSQL_mysql( $db_info->user , $db_info->password , $db_info->db , $db_info->server );
      if ( ! ( $db->quick_connect( $db_info->user , $db_info->password , $db_info->db , $db_info->server ) ) ) {
        throw new Exception( 'Baseball Tools Error</b>:: Cannot connect to the database.' );
      }
      $this->db = $db;
      return true;
    }
    ...
    protected function _get_player_info() {
    	$player_type_query_template = 'select sum( G ) from Batting where playerID="%s" group by playerID';
    	$player_type_query = sprintf( $player_type_query_template , $this->player_id );
    	$gameF = $this->db->get_var( $player_type_query );
    	$player_type_query_template = 'select sum( G ) from Pitching where playerID="%s" group by playerID';
    	$player_type_query = sprintf( $player_type_query_template , $this->player_id );
    	$gameP = $this->db->get_var( $player_type_query );
    	$gameF = ( int )$gameF;
    	$gameP = ( int )$gameP;
    	if ( $gameF + $gameP == 0 ) {
    		return false;
  ...
  /**
      ...
      public function get_age() {
          ...
          if ( is_numeric( $this->birth_year ) 
            ...
            return ( date( "md" , 
                    date( "U" , 
                    mktime( 0 , 0 , 0 , $date_array[0] , $date_array[1] , $date_array[2] ) ) ) > date( "md" ) ? ( ( date( "Y" )-$date_array[2] )-1 ):( date( "Y" )-$date_array[2] ) );
          }
          return false;
      }
  ...
  /**
    ...
    public function get_height() {
      if ( $this->height ) {
        return Basball_tools_helper::convert_height( $this->height );
      }
      return false;
    }    
    ...
    public function get_stats( $year=0 ) {
      ...
      $output=array();
    	...
    	if ( $this->player_type=="F" ) {
    		...
    		$stints                = $this->db->get_var( $query_stats );
    		...
    		$stints_array          = $this->db->get_results( $query_stats , ARRAY_A );
      ...
      if ( $stints > 1 ) {
      		$query_stats_total_template='SELECT "TOT" as teamID , yearID , "-" as lgID , SUM( G ) as G , SUM( G_batting ) as G_batting , SUM( AB ) as AB , SUM( R ) as R , SUM( H ) as H , SUM( 2B ) as 2B , SUM( 3B ) as 3B , SUM( HR ) as HR , SUM( RBI ) as RBI , SUM( SB ) as SB , SUM( CS ) as CS , SUM( BB ) as BB , SUM( SO ) as SO , SUM( IBB ) as IBB , SUM( HBP ) as HBP , SUM( SH ) as SH , SUM( SF ) as SF , SUM( GIDP ) as GIDP , SUM( G_old ) as G_old , if( sum( AB )>0 , format( sum( H )/sum( AB ) , 3 ) , format( 0 , 3 ) ) as AVG , if( sum( AB )>0 , format( ( SUM( H )-( SUM( 2B )+SUM( 3B )+SUM( HR ) )+( SUM( 2B )*2 )+( SUM( 3B )*3 )+( SUM( HR )*4 ) )/SUM( AB ) , 3 ) , format( 0 , 3 ) ) as SLG , if( ( sum( AB )+sum( BB )+sum( HBP )+sum( SF ) )>0 , format( ( sum( H )+sum( BB )+sum( HBP ) )/( ( sum( AB )+sum( BB )+sum( HBP )+sum( SF ) ) ) , 3 ) , format( 0 , 3 ) ) as OBP , format( ( ( SUM( H )-( SUM( 2B )+SUM( 3B )+SUM( HR ) )+( SUM( 2B )*2 )+( SUM( 3B )*3 )+( SUM( HR )*4 ) )/SUM( AB ) ) +( ( sum( H )+sum( BB )+sum( HBP ) )/( ( sum( AB )+sum( BB )+sum( HBP )+sum( SF ) ) ) ) , 3 ) AS OPS from Batting where playerID="%s"  AND yearID=%d GROUP BY yearID';
    			...
    			$total_array = $this->db->get_results( $query_stats_total , ARRAY_A );
